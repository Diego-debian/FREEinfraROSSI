...
class App:
    def Modulo(self):
        bicho = Tk()
        bicho.geometry("430x430+180+60")
        bicho.config(bg="white")
        bicho.title("Infrarrosi")
        bicho.resizable(width=0, height=0)
           
        def Salir():
	    tkMessageBox.showinfo("Infrarrosi", message= "! Cerrando el programa ¡")
        ...
        def Verifica():
	    tkMessageBox.showinfo("Infrarrosi", message= "! Conectando con el dispositivo, por favor espere ¡")
            os.system('xterm -T Infrarrosi -geom 50x8+185+100 +cm -bg blue -e sh c_Blu.sh ')
	    Conecta()
	...
	def Conectar():
            tkMessageBox.showinfo("Infrarrosi", message= "! Verificando conexion ¡")
	    print self.puerta
	    try:
		...
		time.sleep(4)
                ...
                tkMessageBox.showinfo("Infrarrosi", message= "! No hay conexion ¡")
               # os.system('xterm -T Infrarrosi -geom 50x8+185+100 +cm -bg blue -e sh c_Blu.sh &')
...
# ................. Botones menu de inicio ....................................
	    x1 = int(100)
            ...
            btnComenzar = Button(bicho, text= "Difraccción", width=6, height=1, command= Difraccion).place(x=20+x1, y=320)
            btnDetener= Button(bicho, text= "Atenuación", width=6, height=1, command= Atenuacion).place(x=20+x1, y=350)
	    btnLimpiar = Button(bicho, text= "Absorción", width=6, height=1, command= Absorcion).place(x=20+x1, y=380) 	 
...
#------------------------------------- Funcion limpiar pantalla ---------------------------------------------------
	    ...
	    os.system("rm MAC.txt MACD.txt puerto.txt conexion.txt direccion.txt")
	    os.system("sh bin/d_Blu.sh &")
	    os.system("python infrarrosi.py &")
	    os.system("rm MAC.txt MACD.txt puerto.txt")
	    exit()

#---------------------------------- Funciones menu           -----------------------------------------------------
        def Difraccion():
	    tkMessageBox.showinfo("Infrarrosi", message= "Espere por favor,  Preparando todo para empezar con el experimento.")
...
#	    os.system("xterm -T Infrarrosi -geom 50x8+185+100 +cm -bg blue -e python  bin/Atenuacion.py & ")
	...
	def Absorcion():
	    tkMessageBox.showinfo("Infrarrosi", message= "Para detener la visualización de datos, cierre la ventana de datos 'color azul' u oprima el botón reset del Dispositio ")


#---------------------------------- Bluetooth desconectado ----------------------------
	def Bl_off():
	    tkMessageBox.showinfo("Infrarrosi", message= "!Bluetooth desconectado¡")
	    os.system("rm MAC.txt MACD.txt puerto.txt conexion.txt direccion.txt")
	    os.system("sh bin/d_Blu.sh &")
	    os.system("python infrarrosi.py &")
	    os.system("rm MAC.txt MACD.txt puerto.txt")
...
# ------------------------------ Definiendo Funcion firmware -----------------------------
	...
	def Firmware():
	    tkMessageBox.showinfo("Infrarrosi", message= "Conecte la tarjeta microcontroladora arduino uno, con un microcontrolador listo para su uso.\n\nProcediendo con el instalador del firmware")
...
#---------------------- Botones firmware -----------------------------------
	...
	btnDocumentacion=Button(bicho, text = "Ayuda", command=Documentacion, height=1, width =5).place(x=340, y=350)
        btnLimpiar = Button(bicho, text= "limpiar", width=5, height=1, command= Reset).place(x=340, y=320) 	 
        bicho.mainloop()  
