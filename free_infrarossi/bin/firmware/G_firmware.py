...
class G_firmware:
    def Difraccion(self):
	def Salir():
	    tkMessageBox.showinfo("free_infrarrosi", message= "! Cerrando el programa ¡")
            exit() 
	
	def Continuar():
	    tkMessageBox.showinfo("free_infrarrosi", message= "Cargando el firmware en la tarjeta microcontroladora. \n\n si no carga el firmware: \n\n* Revise su conexión con la tarjeta programadora arduino uno. \n\n* Revise el microcontrolador que este funcionando bien ")
	    os.system("python bin/firmware/firmware_free_infrarrosi.py &")

	bicho = Tk()
        bicho.geometry("280x400+507+60")
        bicho.config(bg="white")
        bicho.title("free_infrarrosi")
